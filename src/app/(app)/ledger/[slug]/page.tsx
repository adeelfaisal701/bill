...
export default function LedgerDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="min-h-screen w-full min-w-0 bg-[#f7f9fc] pb-8 text-[#24324a]">
      <div className="flex items-center justify-between gap-4 border-b border-[#e5ebf3] px-4 py-5 sm:px-6 lg:px-8">
        <Link href="/ledger" className="inline-flex items-center gap-1.5 text-sm font-medium text-[#58708f] hover:text-brand-600"><ArrowLeft size={16} /> Back to Ledger</Link>
        <div className="flex items-center gap-2"><button type="button" onClick={() => window.print()} className="inline-flex items-center gap-2 rounded-lg border border-[#dce4ef] bg-white px-4 py-2.5 text-sm font-semibold text-[#435875] shadow-sm"><Printer size={16} /> Print</button><button type="button" onClick={() => window.print()} className="inline-flex items-center gap-2 rounded-lg border border-[#dce4ef] bg-white px-4 py-2.5 text-sm font-semibold text-[#435875] shadow-sm"><Download size={16} /> Download PDF</button></div>
      </div>
      <div className="px-4 pt-6 sm:px-6 lg:px-8"><p className="flex items-center gap-2 text-[28px] font-bold leading-tight tracking-[-0.02em]"><span className="h-3 w-3 rounded-full bg-[#1eaaa5]" />{account.name}</p><p className="mt-1 text-sm text-[#75869e]">Ledger Account - Transaction History</p></div>

      <div className="space-y-6 px-4 pt-6 sm:px-6 lg:px-8">
        <section className="rounded-2xl border border-[#e0e7f0] bg-white p-5 shadow-[0_8px_24px_rgba(30,55,90,0.06)]"><div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          <div className="flex gap-4"><div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full border-2 border-[#263c5c] text-[#263c5c]"><FileText size={24} /></div><div className="text-[15px] leading-7 text-[#50627c]"><p className="mb-2 text-[18px] font-bold leading-6 text-[#24324a]">{account.name}</p><Info label="Account Code" value={account.accountCode || "-"} /><Info label="Type" value={account.type} /><Info label="Project Name" value={account.projectName || "-"} /><Info label="Project Code" value={account.projectCode || "-"} /></div></div>
          <div className="border-t border-[#e4eaf2] pt-4 text-[15px] leading-7 text-[#50627c] md:border-l md:border-t-0 md:pl-6 md:pt-0"><Info label="Contact Details" value={account.contactDetails || "-"} /><Info label="Account Created" value={formatDate(account.createdAt)} /><Info label="Last Updated" value={formatDate(account.updatedAt)} /><Info label="Opening Balance" value={`${formatCurrency(account.openingBalance)} ${account.openingBalanceType === "debit" ? "Dr" : "Cr"}`} /></div>
          <div className="rounded-xl bg-[#f1f6ff] px-4 py-4 text-[15px] leading-7 text-[#50627c] md:col-span-2 xl:col-span-1"><p className="mb-2 text-[18px] font-semibold leading-6 text-[#344b69]">Tax Information</p><Info label="Tax Enabled" value={account.taxEnabled ? "Yes" : "No"} /><Info label="Tax Name" value={account.taxName || "-"} /><Info label="Tax Rate" value={`${account.taxRate ?? 0}%`} /></div>
        </div></section>

        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4"><MetricCard icon={<ArrowUp size={20} />} label="Total Debit" value={formatCurrency(summary.totalDebit)} tone="debit" /><MetricCard icon={<ArrowDown size={20} />} label="Total Credit" value={formatCurrency(summary.totalCredit)} tone="credit" /><MetricCard icon={<ListFilter size={20} />} label="Current Balance" value={`${formatCurrency(summary.currentBalance)} Dr`} tone="balance" /><MetricCard icon={<FileText size={20} />} label="Total Transactions" value={String(summary.totalTransactions)} tone="transactions" /></div>

        <section className="overflow-hidden rounded-2xl border border-[#e0e7f0] bg-white shadow-[0_8px_24px_rgba(30,55,90,0.06)]"><div className="grid gap-3 border-b border-[#e5ebf3] p-4 lg:grid-cols-[minmax(240px,1fr)_repeat(5,minmax(130px,auto))] lg:items-center"><label className="relative min-w-0"><Search size={16} className="absolute left-3 top-3 text-[#91a0b5]" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search by voucher, description..." className="h-11 w-full rounded-lg border border-[#dce4ef] pl-10 pr-3 text-sm outline-none focus:border-brand-400" /></label><FilterSelect value={transactionType} onChange={setTransactionType} options={[["all", "All Transactions"], ["bill", "Bills"], ["payment", "Payments"], ["opening", "Opening"]]} /><FilterSelect value={paymentMode} onChange={setPaymentMode} options={[["all", "Payment Mode"], ["Bank", "Bank"], ["Cash", "Cash"], ["Cheque", "Cheque"], ["Opening", "Opening"]]} /><DateFilter icon={<CalendarDays size={16} />} label="From" value={fromDate} onChange={setFromDate} /><DateFilter label="To" value={toDate} onChange={setToDate} /><div className="flex gap-2"><button type="button" onClick={applyFilters} className="h-11 rounded-lg bg-brand-500 px-6 text-sm font-bold text-white">Apply</button><button type="button" onClick={resetFilters} className="h-11 rounded-lg border border-[#dce4ef] bg-white px-5 text-sm font-semibold text-[#516782]">Reset</button></div></div>
          <div className="overflow-x-auto"><table className="w-full min-w-[900px] border-collapse text-sm text-[#435875]"><thead className="sticky top-0 z-10"><tr className="bg-[#eef3f9] text-left text-xs font-bold text-[#344b69]"><th className="w-16 px-4 py-3">Sr. No.</th><th className="px-4 py-3">Date</th><th className="px-4 py-3">Voucher / Bill #</th><th className="px-4 py-3">Payment Mode</th><th className="px-4 py-3">Description</th><th className="px-4 py-3 text-right">Debit (Rs.)</th><th className="px-4 py-3 text-right">Credit (Rs.)</th><th className="px-4 py-3 text-right">Balance (Rs.)</th></tr></thead><tbody>{transactions.map((transaction, index) => <TransactionRow key={transaction.id} transaction={transaction} index={index} />)}</tbody><tfoot><tr className="bg-[#f1f6ff] font-bold"><td colSpan={5} className="px-4 py-4 text-right">Closing Balance</td><td colSpan={2} /><td className="px-4 py-4 text-right">{formatCurrency(summary.currentBalance)} Dr</td></tr></tfoot></table>{transactions.length === 0 && <p className="px-4 py-8 text-center text-sm text-[#75869e]">No transactions match these filters.</p>}</div>
        </section>
      </div>
    </div>
  );
...
function Info({ label, value }: { label: string; value: string }) { return <p>{label} <span className="mx-2 text-[#a8b4c5]">:</span> {value}</p>; }
...
function MetricCard({ icon, label, value, tone }: { icon: ReactNode; label: string; value: string; tone: "debit" | "credit" | "balance" | "transactions" }) {
  ...
  return <div className="flex min-h-[92px] items-center gap-4 rounded-2xl border border-[#e0e7f0] bg-white p-5 shadow-[0_8px_24px_rgba(30,55,90,0.06)]"><span className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-full ${styles[tone]}`}>{icon}</span><div><p className="text-sm text-[#75869e]">{label}</p><p className="mt-1 text-lg font-bold">{value}</p></div></div>;
}
...
function TransactionRow({ transaction, index }: { transaction: LedgerTransaction; index: number }) {
  return <tr className="hover:bg-[#fafcff]"><td className="border-b border-[#edf1f6] px-4 py-3">{index + 1}</td><td className="whitespace-nowrap border-b border-[#edf1f6] px-4 py-3">{formatDate(transaction.date)}</td><td className="border-b border-[#edf1f6] px-4 py-3 font-semibold text-[#344b69]">{transaction.voucherNumber ? `#${transaction.voucherNumber}` : "-"}</td><td className="border-b border-[#edf1f6] px-4 py-3">{transaction.paymentMode || "-"}</td><td className="border-b border-[#edf1f6] px-4 py-3">{transaction.billId ? <Link href={`/bills/${transaction.billId}`} className="hover:text-brand-600 hover:underline">{transaction.description}</Link> : transaction.description}</td><td className="border-b border-[#edf1f6] px-4 py-3 text-right">{transaction.debit ? transaction.debit.toLocaleString("en-PK") : "-"}</td><td className="border-b border-[#edf1f6] px-4 py-3 text-right">{transaction.credit ? transaction.credit.toLocaleString("en-PK") : "-"}</td><td className="border-b border-[#edf1f6] px-4 py-3 text-right font-medium">{transaction.balance.toLocaleString("en-PK")} Dr</td></tr>;
}
